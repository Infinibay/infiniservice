...
Write-Log "[START] Starting Infiniservice installation..." "SUCCESS"
Write-Log "[PATH] Installation path: $InstallPath" "INFO"
Write-Log "[MODE] Service mode: $ServiceMode" "INFO"
...
Write-Log "[OK] Administrator privileges confirmed" "SUCCESS"
...
if (!(Test-Path $InstallPath)) {
    try {
        ...
        Write-Log "[OK] Created installation directory: $InstallPath" "SUCCESS"
    } catch {
...
if ($SourceExe -and (Test-Path $SourceExe)) {
    try {
        ...
        if ($SourceSize -lt 1048576) {
            Write-Log "[WARNING] Source file seems too small ($SourceSize bytes). May be corrupted or incomplete." "WARNING"
        }
        ...
        if (Test-Path $DestExe) {
            ...
            if ($SourceSize -eq $DestSize) {
                Write-Log "[OK] Successfully copied infiniservice.exe to $DestExe" "SUCCESS"
            } else {
                Write-Log "[WARNING] File sizes don't match - Source: $SourceSize, Dest: $DestSize" "WARNING"
            }
        } else {
            Write-Log "[ERROR] Failed to verify copied executable" "ERROR"
            exit 1
        }
    } catch {
        Write-Log "[ERROR] Failed to copy executable: $_" "ERROR"
        Write-Log "Exception details: $($_.Exception.Message)" "ERROR"
        exit 1
    }
} else {
    Write-Log "[ERROR] Source executable not found in any of the expected locations" "ERROR"
...
try {
    ...
    Write-Log "[OK] Created configuration file: $ConfigPath" "SUCCESS"
} catch {
    Write-Log "[ERROR] Failed to create configuration file: $_" "ERROR"
    exit 1
}
...
if ($VmId) {
    try {
        ...
        Write-Log "[OK] Set VM ID environment variable: $VmId" "SUCCESS"
    } catch {
        Write-Log "[ERROR] Failed to set VM ID environment variable: $_" "ERROR"
    }
} else {
...
if ($ServiceMode -eq "ping-pong") {
    try {
        ...
        Write-Log "[OK] Set service mode to ping-pong" "SUCCESS"
    } catch {
        Write-Log "[ERROR] Failed to set service mode environment variable: $_" "ERROR"
    }
} else {
...
if ($ExistingService) {
    Write-Log "[UPDATE] Found existing service - Status: $($ExistingService.Status)" "WARNING"
...
if ($ServiceArgs) {
    ...
    $ServiceCommand = "`"$DestExe`""
}

Write-Log "[CONFIG] Creating Windows service..." "INFO"
...
if ($LASTEXITCODE -eq 0) {
    Write-Log "[OK] Service created successfully" "SUCCESS"
    ...
    if ($VerifyService) {
        ...
        Write-Log "[WARNING] Service created but verification failed" "WARNING"
    }
} else {
    Write-Log "[ERROR] Failed to create service: $CreateResult" "ERROR"
...
try {
    ...
    if (Test-Path $DestExe) {
        ...
        Write-Log "Testing executable briefly (non-service mode)..." "DEBUG"
        try {
            # Alternative approach: Use System.Diagnostics.Process for better control
            $pinfo = New-Object System.Diagnostics.ProcessStartInfo
            $pinfo.FileName = $DestExe
            # Use --help flag which is guaranteed to exit quickly (--version also works now)
            ...
            Write-Log "Starting test process with --help flag..." "DEBUG"
            $started = $p.Start()
            
            if ($started) {
                # Set a timeout for reading output (prevents indefinite blocking)
                ...
                if ($exited) {
                    ...
                    Write-Log "Test execution completed with exit code: $($p.ExitCode)" "DEBUG"
                    
                    # Try to get output if available
                    if ($outputTask.IsCompleted) {
                        $output = $outputTask.Result
                        if ($output) {
                            Write-Log "Test output (first 500 chars): $($output.Substring(0, [Math]::Min(500, $output.Length)))" "DEBUG"
                        }
                    }
                    ...
                    if ($errorTask.IsCompleted) {
                        ...
                        if ($errorOutput) {
                            Write-Log "Test error output: $errorOutput" "DEBUG"
                        }
                    }
                } else {
                    Write-Log "Test execution timeout after 2 seconds - terminating test" "DEBUG"
                    try {
                        $p.Kill()
                        Write-Log "Test process terminated" "DEBUG"
                    } catch {
                        Write-Log "Could not terminate test process: $_" "DEBUG"
                    }
                }
                
                $p.Dispose()
            } else {
                Write-Log "Failed to start test process" "DEBUG"
            }
        } catch {
            Write-Log "Could not test executable directly: $_" "DEBUG"
    ...
    if ($ServiceStatus.Status -eq "Running") {
        Write-Log "[OK] Infiniservice started successfully!" "SUCCESS"
        ...
        Write-Log "[WARNING] Service created but not running. Status: $($ServiceStatus.Status)" "WARNING"
...
Write-UninstallLog "[STOP] Uninstalling Infiniservice..." "WARNING"
...
[Environment]::SetEnvironmentVariable("INFINIBAY_VM_ID", [NullString]::Value, "Machine")
[Environment]::SetEnvironmentVariable("INFINISERVICE_MODE", [NullString]::Value, "Machine")

# Remove installation directory
Write-UninstallLog "Removing installation directory: $InstallPath" "INFO"
Remove-Item -Path "$InstallPath" -Recurse -Force -ErrorAction SilentlyContinue

Write-UninstallLog "[OK] Infiniservice uninstalled successfully!" "SUCCESS"
Write-UninstallLog "=== UNINSTALLATION COMPLETED ===" "INFO"
"@

try {
    Set-Content -Path $UninstallScript -Value $UninstallContent
    Write-Log "[OK] Created uninstall script: $UninstallScript" "SUCCESS"
} catch {
    Write-Log "[WARNING] Failed to create uninstall script: $_" "WARNING"
}

# Display service status
Write-Log "`n[STATUS] Final Service Status:" "INFO"
...
if ($FinalService) {
    ...
    Write-Log "[ERROR] Could not retrieve final service status" "ERROR"
}
...
if ($FinalService -and $FinalService.Status -eq "Running") {
    Write-Log "[SUCCESS] Infiniservice installation completed successfully!" "SUCCESS"
} else {
    Write-Log "[WARNING] Installation completed but service is not running" "WARNING"
    Write-Log "Please check the log files in C:\Temp for details" "WARNING"
}

Write-Log "[INFO] Configuration file: $ConfigPath" "INFO"
Write-Log "[UNINSTALL] To uninstall, run: $UninstallScript" "INFO"

if ($ServiceMode -eq "ping-pong") {
    Write-Log "[PING-PONG] Service is running in PING-PONG test mode" "WARNING"
